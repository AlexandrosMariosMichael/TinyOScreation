...
typedef enum { 
    IO,   //interactive thread
    QUAN, //time consuming thread
    OTHER //sleep release/mutex/idle thread.
  } yield_cause;
...
typedef struct thread_control_block
{
  ...
  int priority;          /**<The current priority of the thread, starting from 0 when first insterted to the scheduler queue*/
