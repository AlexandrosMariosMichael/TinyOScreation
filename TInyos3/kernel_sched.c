...
volatile unsigned int boost_counter = 0;
...
rlnode SCHED[3];                      /* The scheduler queue */
...
TCB* sched_queue_select()
{
  ...
  rlnode * sel;
  if(rlist_len(&SCHED[0]) != 0)
  {
    sel = rlist_pop_front(& SCHED[0]); 
  }
  else if(rlist_len(&SCHED[1]) != 0)
  {
    sel = rlist_pop_front(& SCHED[1]);
  }
  else if(rlist_len(&SCHED[2]) != 0)
  {
...
void boost_sched()   
{
  ...
  rlist_prepend(&SCHED[0], &SCHED[1]);
  rlist_prepend(&SCHED[0], &SCHED[2]);  
  Mutex_Unlock(& sched_spinlock);
}    
...
{ 
  ...
  switch(current->cause)
  {
    ...
    default:
      fprintf(stderr, "BAD YIELD CAUSE for current thread %p in yield: %d\n", current, current->cause);
  }
...
void gain(int preempt)
{
  ...
  if(current->priority == 0)      // ++++++++++++++++++
    ...
    bios_set_timer(QUANTUM*2);    //  |
  else if(current->priority == 2) //  |
    bios_set_timer(QUANTUM*4);    // ++++++++++++++++++
}
